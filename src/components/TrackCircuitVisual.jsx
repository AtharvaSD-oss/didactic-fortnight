...
const TrackCircuitVisual = () => {
  ...
  return (
    <div className="relative w-full rounded-2xl bg-[#090A10] border border-gray-800 p-3 sm:p-5 shadow-2xl overflow-hidden text-white select-none">
      
      {/* Top Telemetry Header Bar */}
      <div className="flex flex-wrap items-center justify-between gap-2 pb-3 mb-3 border-b border-gray-800/80">
        <div className="flex items-center gap-2">
          <span className="w-2.5 h-2.5 rounded-full bg-[#EE3124] animate-pulse" />
        ...
        <div className="flex items-center gap-2 text-[10px] font-mono">
          <button
            ...
            className={`px-2.5 py-1 rounded-full border transition-colors ${
              showRacingLine ? 'bg-[#EE3124]/20 border-[#EE3124] text-[#EE3124]' : 'bg-gray-900 border-gray-800 text-gray-400'
            }`}
          >
          ...
          <button
            ...
            className={`px-2.5 py-1 rounded-full border transition-colors ${
              showSectors ? 'bg-[#FFD700]/20 border-[#FFD700] text-[#FFD700]' : 'bg-gray-900 border-gray-800 text-gray-400'
            }`}
          >
      ...
      <div className="relative w-full aspect-[16/10] max-h-[460px] bg-gradient-to-br from-[#0e1017] via-[#090a0f] to-[#06070a] rounded-xl overflow-hidden flex items-center justify-center p-2 border border-gray-800/60">
        ...
        >
          ...
          {/* 3. Red & Green Racing Kerbs at Apexes (matches schematic) */}
          <g>
            {/* Top-Right Sweeper Kerb */}
          ...
          {TURN_DATA.map((turn) => (
            ...
            >
              <circle
                ...
                cy={turn.y}
                r="16"
                fill={activeTurn?.id === turn.id ? "#EE3124" : "rgba(17, 18, 23, 0.9)"}
                stroke={activeTurn?.id === turn.id ? "#FFD700" : "#EE3124"}
                strokeWidth="2.5"
        ...
        <AnimatePresence>
          {activeTurn && (
            <motion.div
              ...
              className="absolute bottom-3 left-3 right-3 sm:right-auto sm:max-w-xs bg-black/90 border border-[#EE3124]/60 backdrop-blur-xl p-3.5 rounded-xl shadow-2xl z-30"
            >
              <div className="flex items-center justify-between border-b border-gray-800 pb-1.5 mb-1.5">
                <span className="font-display font-bold text-xs text-[#FFD700] uppercase">
                  {activeTurn.name}
                </span>
                <button
              ...
              <div className="grid grid-cols-2 gap-2 text-[10px] font-mono mb-1.5">
                <div>
                  ...
                  <div className="text-white font-bold">{activeTurn.speed}</div>
                </div>
                <div>
                  <span className="text-gray-400">BRAKING ZONE:</span>
                  <div className="text-[#EE3124] font-bold">{activeTurn.gear}</div>
                </div>
              </div>
              <p className="text-[11px] font-sans text-gray-300 leading-snug">
                {activeTurn.desc}
              </p>
            </motion.div>
          )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-3 text-center font-mono">
        <div className="p-2 rounded-lg bg-gray-900/90 border border-gray-800">
          <div className="text-[9px] text-gray-400 uppercase">CIRCUIT LENGTH</div>
          <div className="text-sm sm:text-base font-display font-bold text-[#EE3124]">720 METERS</div>
        </div>
        <div className="p-2 rounded-lg bg-gray-900/90 border border-gray-800">
          <div className="text-[9px] text-gray-400 uppercase">TECHNICAL TURNS</div>
          <div className="text-sm sm:text-base font-display font-bold text-white">14 APEXES</div>
        </div>
        <div className="p-2 rounded-lg bg-gray-900/90 border border-gray-800">
          <div className="text-[9px] text-gray-400 uppercase">SURFACE TYPE</div>
          <div className="text-sm sm:text-base font-display font-bold text-[#FFD700]">POLYMER ASPHALT</div>
        </div>
        <div className="p-2 rounded-lg bg-gray-900/90 border border-gray-800">
          <div className="text-[9px] text-gray-400 uppercase">SAFETY SYSTEM</div>
          <div className="text-sm sm:text-base font-display font-bold text-white">F1 PRO BARRIER</div>
        </div>
      </div>
