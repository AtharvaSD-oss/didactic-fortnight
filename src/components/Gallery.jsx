...
const cards = [
  ...
  },
  {
    ...
    rotation: 'rotate-0',
    scale: 'scale-105 hover:scale-110 shadow-2xl border-[#FFB800] z-20',
    featured: true,
  },
  {
    id: 'thrill',
...
const Gallery = () => {
  ...
  return (
    <section id="gallery" className="py-28 px-4 sm:px-8 lg:px-12 bg-[#FFFFFF] relative z-20 border-t border-[#E8E8E8] select-none">
      <div className="max-w-7xl mx-auto space-y-16">
        ...
        <div className="text-center space-y-3 max-w-2xl mx-auto">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#FFB800]/15 border border-[#FFB800]/40 text-[#FF5A1F] font-sans text-xs font-bold uppercase tracking-widest">
            <Sparkles className="w-4 h-4 text-[#FF5A1F]" /> VISUAL SHOWCASE
          </div>
          <h2 className="text-3xl sm:text-5xl font-display font-black text-[#111111] uppercase tracking-tight">
            THE ARENA <span className="text-[#FFB800]">GALLERY</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-6 items-center">
          {cards.map((card, idx) => (
            <motion.div
              ...
              className={`
                relative group rounded-[24px] overflow-hidden cursor-pointer border border-[#E8E8E8] hover:border-[#FFB800]
                transition-all duration-500 shadow-[0_15px_40px_rgba(0,0,0,0.06)] hover:shadow-[0_25px_60px_rgba(0,0,0,0.12)] bg-white
            ...
            >
              ...
              <motion.img
                src={card.image}
                alt={card.title}
                className="absolute inset-0 w-full h-full object-cover object-center group-hover:scale-110 transition-transform duration-700 filter brightness-100 contrast-105"
              />
              ...
              <div className="absolute top-4 left-4 z-20">
                <span className="px-3.5 py-1 rounded-full bg-white/90 border border-white/40 text-[#111111] font-mono text-[10px] font-bold uppercase tracking-widest backdrop-blur-md shadow-sm">
                  {card.badge}
                </span>
              </div>

              {/* Zoom Icon */}
              <div className="absolute top-4 right-4 z-20 p-2.5 rounded-full bg-white/80 border border-white/50 text-[#111111] group-hover:bg-[#FFB800] transition-colors duration-300 shadow-sm">
                <Maximize2 className="w-4 h-4" />
              </div>

              {/* Bottom Left Title */}
              <div className="relative z-20 p-6 space-y-1 text-left">
                <h3 className="text-2xl sm:text-3xl font-display font-semibold text-white uppercase tracking-tight group-hover:text-[#FFB800] transition-colors">
                  {card.title}
                </h3>
        ...
        {selectedImage && (
          ...
          >
            <motion.div
              ...
              className="relative max-w-4xl w-full bg-white border border-[#E8E8E8] rounded-3xl overflow-hidden shadow-2xl"
            >
              <button
                onClick={() => setSelectedImage(null)}
                className="absolute top-4 right-4 z-30 p-3 rounded-full bg-white/90 text-[#111111] hover:text-[#FF5A1F] border border-[#E8E8E8] transition-colors shadow-md"
              >
                <X className="w-5 h-5" />
              </button>

              <img src={selectedImage.image} alt={selectedImage.title} className="w-full max-h-[75vh] object-contain bg-white" />

              <div className="p-6 bg-[#F8F9FB] border-t border-[#E8E8E8] flex items-center justify-between">
                <div>
