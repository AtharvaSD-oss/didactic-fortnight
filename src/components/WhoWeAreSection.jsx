...
const StatCounter = ({ end, suffix = '', label }) => {
  ...
  return (
    <div ref={ref} className="p-5 sm:p-6 rounded-2xl bg-white border border-gray-200/90 text-center space-y-1 shadow-sm hover:shadow-md hover:border-[#EE3124]/40 transition-all duration-300 group">
      <div className="text-2xl sm:text-3xl font-display font-black text-[#0B0F19] group-hover:text-[#EE3124] transition-colors">
        {count}{suffix}
      </div>
      <div className="font-mono text-[9px] sm:text-[10px] text-gray-500 uppercase tracking-wider font-semibold">
        {label}
      </div>
    </div>
  );
...
const WhoWeAreSection = () => {
  ...
  return (
    <section
      ref={sectionRef}
      id="who-we-are"
      className="py-14 sm:py-18 px-4 sm:px-8 lg:px-12 max-w-7xl mx-auto border-t border-gray-200/80 relative z-20 text-gray-900 bg-white"
    >
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-center mb-10">
        
        {/* Left Side Image */}
        <div className="lg:col-span-6 relative">
          <div className="relative rounded-2xl overflow-hidden border border-gray-200 shadow-xl bg-gray-100 group">
            <img
              ref={imgRef}
              src="/about-action.jpg"
              alt="Kartomania & Leap Frog Racing Kart #71"
              data-cursor="VIEW"
              className="w-full h-[340px] sm:h-[420px] object-cover object-center group-hover:scale-105 transition-transform duration-700"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-transparent to-transparent opacity-80" />
            <div className="absolute bottom-4 left-4 right-4 p-3.5 rounded-xl bg-white/90 backdrop-blur-md border border-gray-200 flex items-center justify-between shadow-md">
        ...
        {/* Right Side Text */}
        <div ref={textRef} className="lg:col-span-6 space-y-4 text-left">
